<template>
  <Container :class="['t-container', page.fields.slug]" accessible-line-length="true">
    <DynamicHeadline :id="page.fields.title | idAlize" slot="headline" :level="1" class="p-name">
      {{ page.fields.title }}
    </DynamicHeadline>
    <div class="c-tile">
      <div class="c-tile__container">
        <p v-if="page.fields.isResource">
          <strong>Last updated at: <PrettyDate v-if="page.fields.isResource" slot="date" :date="page.sys.updatedAt" /></strong>
        </p>
        <Marked :markdown="page.fields.body" class="e-content" />
      </div>
    </div>
  </Container>
</template>
